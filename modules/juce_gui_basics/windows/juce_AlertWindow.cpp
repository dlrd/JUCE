...
void AlertWindow::updateLayout (const bool onlyIncreaseSize)
{
    ...
    auto w = /*jmin */ jmax (300 + sw * 2, (int) ((float) getParentWidth() * 0.7f)); // SMODE avoid to reduce AlertWindow width
    ...
    // w = jmin (w, (int) ((float) getParentWidth() * 0.7f)); // SMODE avoid to reduce AlertWindow width
